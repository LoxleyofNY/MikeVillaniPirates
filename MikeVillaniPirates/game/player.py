...
class Player (Context):
    ...
    def __init__ (self, world, ship):
        ...
        self.inventory = []
        self.scroll_read = False ##need to change based for scroll function to work
    ...
    def equip_item(self, item):
        ...
        print(f"You have the equipped the {item.name}.")
        ...
        if (verb == "quit"):
            ...
            if len(cmd_list) > 1: #storing whats after read into list
                ...
                if not (item_found is None):
                    print(f"You read the {item_found.name}")
                    print(f"{item_found.description}")
        
                    if item_name == "scroll" or "crystal_key" or "jade_key" or "copper_key":
                        response_inscription = input(f"Do you want to read the inscription on the {item_name}? (yes/no): ")
                        if response_inscription == "yes" or response_inscription == "y":
                            print(f"You carefully read the inscription on the {item_name}.")
                            print(f"{item_found.inscription}")
                            #self.scroll_read = True ##need to change based for scroll function to work
                        else:
                            print("You decide not to read the inscription.")
                else:
                    print(f"You don't have the {item_name} or it's not readable.")
        elif (verb == "go"):
            self.go = True
            if (len(cmd_list) > 1):
                if (cmd_list[1] == "north"):
